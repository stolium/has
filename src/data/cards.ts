import { Card, CastingCostAction } from './types';

function makeCurse(id: string, name: string, costLabel: string, effect: string, costAction?: CastingCostAction): Card {
  return { id, type: 'curse', name, castingCost: { label: costLabel, action: costAction }, effect };
}
...
const CURSES: Card[] = [
  makeCurse('curse-move', 'Curse of the Move Card', 'Discard 2 cards', 'Gives the hider 45–60 minutes to pack up and completely change locations.', { kind: 'cards', count: 2 }),
  ...
  makeCurse('curse-drained-brain', 'Curse of the Drained Brain', 'Discard your entire hand', 'Hider chooses 3 questions; seekers are banned from asking them for the rest of the run.', { kind: 'all' }),
  ...
  makeCurse('curse-prosperous-home', 'Curse of the Prosperous Home', 'Discard 20+ minutes of time bonuses', "Permanently expands the hider's zone radius by 50%. (Removed in endgame).", { kind: 'time_bonus_minutes', minMinutes: 20 }),
  ...
  makeCurse('curse-zoologist', 'Curse of the Zoologisдуеt', 'Take a photo of a wild animal', 'Seekers must find and photograph an animal in that same biological category.'),
